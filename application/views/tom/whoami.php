    ...
    <main>
      <section>
        ...
        <div class="pdf-download">
            <button class="black" @click="pdfDown"><a href="javascript:;">이력서 PDF 다운</a></button>
            <button class="gray" @click="pdfDown2"><a href="javascript:;">경력기술서 PDF 다운</a></button>
        </div>
      ...
      <section>
        ...
        <ol>
          ...
          <li>
            ...
            <ul>
              <li>주 사용환경은 Spring boot, Mybatis, PHP7, 내부 프레임워크, Vue2, Vue3 Cli, MariaDB, CentOS, Apache, Nginx, 네이버클라우드
                호스팅 기반으로 개발하였습니다.</li>
...
<script>
  const vm = new Vue({
    ...
    methods: {
      ...
      pdfDown() {
        axios.get(`${window.location.origin}/upload/etc/이대호_이력서_2023_04_19.pdf`, {
          responseType: 'blob',
        }).then(response => {
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', '이대호_이력서.pdf');
          document.body.appendChild(link);
          link.click();
        });
      },
      pdfDown2() {
        axios.get(`${window.location.origin}/upload/etc/이대호_경력기술서_2023_03_25.pdf`, {
          responseType: 'blob',
        }).then(response => {
          const url = window.URL.createObjectURL(new Blob([response.data]));
          const link = document.createElement('a');
          link.href = url;
          link.setAttribute('download', '이대호_경력기술서.pdf');
          document.body.appendChild(link);
          link.click();
        });
      },
      pdfDown_make() {
        axios({
          url: '/tom/pdf_whoami',
          method: 'GET',
          responseType: 'blob', // 바이너리 데이터 타입으로 설정
        }).then((response) => {
          const pdfData = response.data; // 바이너리 데이터
          const fileName = new Date().YYYYMMDD() + '_이대호_경력기술서.pdf'; // 다운로드할 파일명
          // Blob 객체 생성
